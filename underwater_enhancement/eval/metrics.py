...
def uiqm(image_rgb: np.ndarray) -> float:
    ...
    rg = r - g
    yb = 0.5 * (r + g) - b
    uicm = -0.0268 * np.sqrt(np.mean(rg) ** 2 + np.mean(yb) ** 2) + 0.1586 * np.sqrt(np.var(rg) + np.var(yb))
    ...
    return float(0.0282 * uicm + 0.2953 * uism + 3.5753 * uiconm + 0.1 * saturation.mean())
