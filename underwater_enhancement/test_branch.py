...
from utils.image_io import image_to_tensor, list_images, pil_loader, read_image_rgb, resize_rgb, save_comparison, save_image_rgb, tensor_to_image
...
def _find_reference_for_physical(image_path: Path, mapping: dict[str, Path], reference_index: dict[str, Path]) -> Path | None:
    ref = mapping.get(image_path.as_posix().lower()) or mapping.get(image_path.name.lower()) or mapping.get(image_path.stem.lower())
    if ref is None:
        return None
    if ref.exists():
        return ref
...
def _save_average(rows: list[dict], average_csv: str | Path) -> pd.DataFrame:
    ...
    grouped = (
        ...
        .agg(PSNR_mean=("PSNR", "mean"), SSIM_mean=("SSIM", "mean"), UIQM_mean=("UIQM", "mean"), UCIQE_mean=("UCIQE", "mean"), num_images=("UIQM", "count"))
        .reset_index()
    )
...
def _evaluate_image(model: torch.nn.Module, image_path: Path, ref_path: Path | None, branch: str, source_type: str, args: argparse.Namespace, device: torch.device) -> dict:
...
def test_one_branch(branch: str, args: argparse.Namespace, device: torch.device, reference_index: dict[str, Path]) -> list[dict]:
    ...
    for image_path in tqdm(real_images, desc=f"{branch} real_classified"):
        ...
        rows.append(_evaluate_image(model, image_path, ref_path, branch, "real_classified", args, device))
    ...
    if physical_images:
        ...
        for image_path in tqdm(physical_images, desc=f"{branch} physical_generated"):
            ...
            rows.append(_evaluate_image(model, image_path, ref_path, branch, "physical_generated", args, device))
...
def run(args: argparse.Namespace) -> pd.DataFrame:
    ...
    df = pd.DataFrame(rows, columns=["branch", "source_type", "image_name", "input_path", "reference_path", "output_path", "PSNR", "SSIM", "UIQM", "UCIQE"])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test pretrained expert branches independently.")
