...
WORKDIR="/root/autodl-tmp/experiments/three_stage_uieb/workdir/v3"
...
python "${SCRIPT_DIR}/test_three_stage.py" \
  ...
  --output-dir "${WORKDIR}/test_results/test_result_stage3" \
...
python "${SCRIPT_DIR}/test_euvp.py" \
  --checkpoint "${CHECKPOINT}" \
  --euvp-root "${EUVP_ROOT}" \
  --output-dir "${WORKDIR}/euvp_test_results/euvp_test_results_stage3" \
  --image-size "${IMAGE_SIZE}" \
  --device "${DEVICE}" 
