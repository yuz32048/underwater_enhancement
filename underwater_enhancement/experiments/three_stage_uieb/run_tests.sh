...
CHECKPOINT="${CHECKPOINT:-${WORKDIR}/checkpoints/stage2/stage2_best.pth}"
DEVICE="${DEVICE:-auto}"
...
python "${SCRIPT_DIR}/test_three_stage.py" \
  ...
  --output-dir "${WORKDIR}/test_results/test_stage2" \
...
python "${SCRIPT_DIR}/test_euvp.py" \
  --checkpoint "${CHECKPOINT}" \
  --euvp-root "${EUVP_ROOT}" \
  --output-dir "${WORKDIR}/euvp_test_results/euvp_test_stage2" \
  --image-size "${IMAGE_SIZE}" \
  --device "${DEVICE}" 
