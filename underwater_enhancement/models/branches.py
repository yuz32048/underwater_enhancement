...
class VGGFeatureBlock(nn.Module):
    def __init__(self):
        ...
        self.vgg = vgg
        self.reduce = nn.Conv2d(128, 16, 1)
...
class BlueCastBranch(nn.Module):
    def __init__(self, channels: int = 32):
        ...
        self.cnn = nn.Sequential(ConvBlock(3 + 16, channels), nn.Conv2d(channels, 3, 3, padding=1), nn.Sigmoid())
...
class LowLightBranch(nn.Module):
    def __init__(self, channels: int = 32, gamma: float = 0.7):
        super().__init__()
        self.gamma = gamma
        self.net = nn.Sequential(ConvBlock(3, channels), nn.Conv2d(channels, 3, 3, padding=1), nn.Sigmoid())

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        gamma_corrected = torch.pow(x.clamp(1e-6, 1), self.gamma)
        return self.net(gamma_corrected)
